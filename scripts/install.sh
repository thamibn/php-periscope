...
# preserved as .bak before being overwritten. Must run AFTER INI_BODY is set
# — the diff requires it (under `set -u` an undefined reference is fatal).
...
if [[ $DRY_RUN -eq 1 ]]; then
  ...
  # On macOS, the first `php -m` immediately after the codesign re-sign can
  # SIGKILL the PHP process while the kernel propagates the new signature.
  # Retry a few times with a short backoff before giving up — by the third
  # attempt the kernel has caught up and dlopen succeeds. Same retry covers
  # the rare case of a stale shared cache after ini changes.
  _verify_ok=0
  for _verify_try in 1 2 3 4 5; do
  ...
  if [[ $_verify_ok -eq 1 ]]; then
    ...
    # Surface the actual reason instead of a generic "did not list" message —
    # 95% of the time it's a dlopen error visible only with startup errors on.
    _verify_err="$("$PHP_BIN" -d display_startup_errors=1 -m 2>&1 | grep -i -E 'warning|error|fatal|undefined' | head -3)"
    if [[ -n "$_verify_err" ]]; then
      warn "php startup reported:"
      printf "      %s\n" "$_verify_err" | sed 's/^      /      /'
    fi
    fail "php -m did NOT list periscope after 5 retries. Inspect $INI_FILE and re-run."
  fi
